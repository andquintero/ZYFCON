...
setwd("~/master_thesis/Arabidopsis/modules/")
load(".RData")
save.image(".RData")
...
edges <- edges[edges$weight >= 0.05,]#filter nodes autopredicting
edges <- edges[edges$weight >= 0.1,]#filter nodes autopredicting
...
#4 steps walktrap 42026 communities usign all edges
#4 steps walktrap 54 communities usign edges > 0.05
...
x <- proportion_score(map_presence, expected_paths) 
...
all_random <- c(fifty_random, one_random, two_random)
names(all_random)
...
write.table(edges_putative_pvalue_order, "results/edgelist_putatice_weighted.txt", 
            col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
...
KO <- read.table("http://rest.kegg.jp/link/enzyme/ath")
