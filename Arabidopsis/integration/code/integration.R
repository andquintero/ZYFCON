...
setwd("~/master_thesis/Arabidopsis/integration/")
load(".RData")
save.image(".RData")
...
write.table(edges_pvaluesum, "results/edgelist_weighted.txt", 
            col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

edges_order_pvaluesum$p.value  <- edges_pvaluesum$p.value
edges_order_pvaluesum$num.p  <- edges_pvaluesum$num.p
################################################################################
