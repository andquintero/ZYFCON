...
setwd("~/master_thesis/s_cerevisiae/GO_similarity_bp/")
oldpar <-par()
load(".RData")
save.image(".RData")
################################################################################
...
sim <- as.matrix(biological_process_GO[gogold$V2,gogold$V2])
dim(sim)
sim[is.na(sim)] <- 0

pdf("results/gold_sim_image_go.bp.pdf", width=4, height=4)
heatmap_2(sim, Rowv=F, Colv=F, do.dendro=c(F, F), col=grey(seq(0, 1, length = 256)), scale="none")
dev.off()

tiff("results/gold_sim_image_go.bp.tiff", width=4, height=4, units="in", res = 300)
heatmap_2(sim, Rowv=F, Colv=F, do.dendro=c(F, F), col=grey(seq(0, 1, length = 256)), scale="none")
dev.off()
