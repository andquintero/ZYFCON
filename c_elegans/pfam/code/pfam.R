...
setwd("~/master_thesis/s_cerevisiae/pfam/")
load(".RData")
save.image(".RData")
...
?getBM
yeast_pfam <- getBM(filters = "biotype",
...
write.csv(yeast_pfam, file="data/yeast_pfam.csv")

################################################################################
##                  Listing and choosing filters gene ID                      ##
################################################################################

yeast_pfam_geneID <- getBM(filters = "biotype",
                  values = "protein_coding",
                  attributes = c("ensembl_gene_id",
                                 "pfam"),
                  mart=ensembl)

#filter genes without pfam annotation
x <- nchar(yeast_pfam_geneID$pfam) == 7
yeast_pfam_geneID <- yeast_pfam_geneID[x,]

################################################################################
##                           transform to binary matrix                       ##
################################################################################

ctable_pfam <- table(yeast_pfam_geneID) #as table
...
z <- as.character(rownames(pfam_Ochiai))
...
sim <- as.matrix(pfam_Ochiai[gogold$V2,gogold$V2])
dim(sim)
sim[is.na(sim)] <- 0

pdf("results/gold_sim_image_pfam.pdf", width=4, height=4)
heatmap_2(sim, Rowv=F, Colv=F, do.dendro=c(F, F), col=grey(seq(0, 1, length = 256)), scale="none")
dev.off()

tiff("results/gold_sim_image_pfam.tiff", width=4, height=4, units="in", res = 300)
heatmap_2(sim, Rowv=F, Colv=F, do.dendro=c(F, F), col=grey(seq(0, 1, length = 256)), scale="none")
dev.off()
